...
{
    public partial class DevicesThatWereDelivered : UserControl
    {
        ...
        int pageSize = 50;
        int totalPages;
        public DevicesThatWereDelivered()
        {
            InitializeComponent();
        }

        private void DevicesThatWereDelivered_Load(object sender, EventArgs e)
        {
        ...
        private void LoadDeliveredDevices()
        {
            DateTime? from = GetFromDate();
            DateTime? to = GetToDate();

            DataTable dt = reportsBLL.GetDeliveredDevices(from, to, currentPage, pageSize);
            dataGridView1.DataSource = dt;

            int totalRows = reportsBLL.GetDeliveredDevicesCount(from, to);
